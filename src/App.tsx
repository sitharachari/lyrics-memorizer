...
function PracticeSession({ verses, onExit, onComplete }: { verses: string[][], onExit: () => void, onComplete: (stats: SessionStats) => void }) {
  ...
  useEffect(() => { inputRef.current?.focus(); }, [currentLineIndex, currentVerseIndex]);
  ...
  const handleTyping = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value[value.length - 1] === targetLine[value.length - 1]) setCorrectKeystrokes(prev => prev + 1);
    else if (value.length <= targetLine.length) setCurrentLineErrors(prev => prev + 1);

    if (value === targetLine) {
      if (currentLineErrors > 2) setFailedVerseIndices(prev => new Set(prev).add(currentVerseIndex));
      setTimeout(() => {
        if (currentLineIndex < verses[currentVerseIndex].length - 1) {
          setCurrentLineIndex(prev => prev + 1); setUserInput(''); setCurrentLineErrors(0);
        } else if (currentVerseIndex < verses.length - 1) {
          setCurrentVerseIndex(prev => prev + 1); setCurrentLineIndex(0); setUserInput(''); setCurrentLineErrors(0);
        } else {
          onComplete({
            wpm: Math.round((correctKeystrokes / 5) / ((Date.now() - startTime!) / 60000)),
            accuracy: Math.round((correctKeystrokes / totalKeystrokes) * 100),
            failedVerses: Array.from(failedVerseIndices).map(idx => verses[idx])
          });
        }
      }, 150); 
    }
  ...
  return (
    <div className="container practice-container" onClick={() => inputRef.current?.focus()}>
      ...
      <div className="typing-container">
        <input ref={inputRef} className="hidden-input" value={userInput} onChange={handleTyping} autoComplete="off" spellCheck="false" />
        {targetLine.split('').map((char, index) => (
          <span key={index} className={`char ${index < userInput.length ? (userInput[index] === char ? 'correct' : 'incorrect') : ''}`}>{char}</span>
        ))}
      </div>
    </div>
  );
